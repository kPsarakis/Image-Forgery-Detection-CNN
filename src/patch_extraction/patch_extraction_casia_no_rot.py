...
class PatchExtractor:
    ...
    def __init__(self, patches_per_image=4, stride=8):
        """
        Initialize class
    ...
    def extract_authentic_patches(self, sp_pic, num_of_patches, rep_num):
        ...
        if sp_name in Au_pic_dict.keys():
            ...
            for i, ind in enumerate(inds):
                io.imsave('patches_casia_no_rot/authentic/{0}_{1}_{2}.png'.format(au_name, i, rep_num), non_tampered_patches[ind])
    ...
    def extract_patches(self):
        ...
        if not os.path.exists('patches_casia_no_rot'):
            os.makedirs('patches_casia_no_rot')
            os.makedirs('patches_casia_no_rot/authentic')
            os.makedirs('patches_casia_no_rot/tampered')
        else:
            if os.path.exists('patches_casia_no_rot/authentic'):
                self.delete_prev_images('patches_casia_no_rot/authentic')
            else:
                os.makedirs('patches_casia_no_rot/authentic')
            if os.path.exists('patches_casia_no_rot/tampered'):
                self.delete_prev_images('patches_casia_no_rot/tampered')
            else:
                os.makedirs('patches_casia_no_rot/tampered')
        # define window shape
        window_shape = (128, 128, 3)
        ...
        for f in os.listdir(tp_dir):
            try:
                ...
                for i, ind in enumerate(inds):
                    io.imsave('patches_casia_no_rot/tampered/{0}_{1}_{2}.png'.format(im_name, i, rep_num), tampered_patches[ind][0])
